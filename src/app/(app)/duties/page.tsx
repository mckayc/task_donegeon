...
export default function DutiesPage() {
  ...
  const activeDuties = useMemo(() => {
    return tasks.filter(task => {
      let passes = task.type === 'duty' && task.status === 'active';
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Filter Duties</CardTitle>
          <CardDescription>
            Find specific duties using the filters below.
          </CardDescription>
        </CardHeader>
      ...
      <div className="space-y-4">
        {activeDuties.length > 0 ? activeDuties.map(task => (
          ...
          <Card className="text-center text-muted-foreground p-8">
            <p>No duties match your current filters.</p>
          </Card>
        )}
      </div>
    </div>
